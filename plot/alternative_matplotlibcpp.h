...
public:
    ...
    Plot(Plot && other){
        std::cout <<  static_cast<void*>(this) << " Plot move constructor\n";
        line = std::move(other.line);
        set_data_fct = std::move(other.set_data_fct);
        if(line){
            Py_INCREF(line);
        }
        if(set_data_fct){
            Py_INCREF(set_data_fct);
        }
        std::cout <<  static_cast<void*>(this) << " Plot move constructor 1\n";
    }
    Plot& operator=(Plot&& other){
        std::cout <<  static_cast<void*>(this) << " Plot move operator\n";
        line = std::move(other.line);
        set_data_fct = std::move(other.set_data_fct);
        if(line){
            Py_INCREF(line);
        ...
        std::cout <<  static_cast<void*>(this) << " Plot move operator 1\n";
        return *this;
    }
    ...
    Plot(const std::string& name, const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::string& format = "") {
        std::cout <<  static_cast<void*>(this) << " Plot Normal constructor\n";
        assert(x.size() == y.size());
        ...
        std::cout << static_cast<void*>(this) << " Plot Normal constructor 1\n";
    }
    ...
    ~Plot() {
        std::cout << static_cast<void*>(this) << " Plot destructor\n";
        decref();
        std::cout << static_cast<void*>(this) << " Plot destructor2\n";
    }
private:
...
public:
    // default initialization with plot label, some data and format
    template<typename Numeric>
    Text(Numeric x, Numeric y, const std::string& text) {
        PyObject* kwargs = PyDict_New();

        PyObject* x_p = PyFloat_FromDouble((double)x);
        PyObject* y_p = PyFloat_FromDouble((double)y);


        PyObject* pystring = PyString_FromString(text.c_str());

        PyObject* plot_args = PyTuple_New(3);
        PyTuple_SetItem(plot_args, 0, x_p);
        PyTuple_SetItem(plot_args, 1, y_p);
        PyTuple_SetItem(plot_args, 2, pystring);

        PyObject* res = PyObject_Call(detail::_interpreter::get().s_python_function_text, plot_args, kwargs);
    ...
    template<typename Numeric>
    bool update(Numeric x, Numeric y) const{
        auto set_position_fct = PyObject_GetAttrString(m_text,"set_position");
        PyObject* x_p = PyFloat_FromDouble((double)x);
        PyObject* y_p = PyFloat_FromDouble((double)y);

        PyObject* new_coordinates = PyTuple_New(2);
        
        PyTuple_SetItem(new_coordinates, 0, x_p);
        PyTuple_SetItem(new_coordinates, 1, y_p);

        PyObject* plot_args = PyTuple_New(1);
        PyTuple_SetItem(plot_args, 0, new_coordinates);


        PyObject* res = PyObject_CallObject(set_position_fct, plot_args);
        if (res) Py_DECREF(res);
        return res;
    }
    ...
    void remove() {
        if(m_text){
            auto remove_fct = PyObject_GetAttrString(m_text,"remove");
            PyObject* args = PyTuple_New(0);
            PyObject* res = PyObject_CallObject(remove_fct, args);
            if (res) Py_DECREF(res);
        }
